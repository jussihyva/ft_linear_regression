...
/*   Updated: 2021/07/23 19:16:00 by jkauppi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_linear_regression.h"

static void	create_stat_counter_string_names(char *string[NUM_OF_STAT_COUNTERS])
{
...
t_statistics	*initialize_statistics(void)
{
	...
	statistics->stat_counters.is_active[E_MAX_NUM_OF_SAVED_PUZZLE_STATES] = 1;
	return (statistics);
}
...
void	check_mem_usage(t_memory_info *memory_info, t_statistics *statistics)
{
	...
	statistics->stat_counters.is_active[E_IS_MEM_LIMIT_REACHED] = 1;
	if (memory_info->av_phys_pages < 50000)
		statistics->stat_counters.value[E_IS_MEM_LIMIT_REACHED] = 1;
...
void	stat_update_mem_usage(t_statistics *statistics)
{
	...
	statistics->stat_counters.is_active[E_IS_TIME_LIMIT_REACHED] = 1;
	if ((EXECUTION_TIME_LIMIT_MIN * MINUTE * MILLI_SECONDS)
		< (clock() / (CLOCKS_PER_SEC / 1000)))
		statistics->stat_counters.value[E_IS_TIME_LIMIT_REACHED] = 1;
	...
	statistics->stat_counters.is_active[E_MAX_MEM_USAGE] = 1;
	statistics->stat_counters.value[E_MAX_MEM_USAGE]
		= ft_max_int(statistics->stat_counters.value[E_MAX_MEM_USAGE],
